...
bool IsGraphImplementedNode(const MaterialX::NodePtr& node)
{
...
{
    // Expand every NodeGraph implementation, not just the high-level surface
    // model. This preserves the authored semantics of helper/value nodes such
    // as anisotropic roughness without renderer knowledge of their names.
    // Source-code implementations remain primitive operations for the closure
    // target to implement directly.
    const MaterialX::NodePredicate predicate =
        [](const MaterialX::NodePtr& node) {
            return IsGraphImplementedNode(node);
        };
    for (const MaterialX::NodeGraphPtr& graph : document->getNodeGraphs()) {
        graph->flattenSubgraphs(MaterialX::EMPTY_STRING, predicate);
    }
    ...
    const MaterialX::TypedElementPtr& renderable)
{
    ...
    // Document::copy intentionally shares its immutable data-library pointer.
    // Full NodeGraph expansion mutates graph instances during flattening, so
    // give the closure compiler an isolated library copy and keep subsequent
    // raster generation/cache keys deterministic.
    if (source->getDataLibrary()) {
        document->setDataLibrary(source->getDataLibrary()->copy());
    }
